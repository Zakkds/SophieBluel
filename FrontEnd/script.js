...
function boutonFiltrer(filter) {
  let filtredWorks = [];
  filtredWorks = works.filter((photo) =>
    photo.category.name.startsWith(filter)
  );
  document.querySelector(".gallery").innerHTML = "";
  createCard(filtredWorks);
  if (filter === "tous") {
    createCard(works);
  }
}
...
//Création évènement "click" sur loginLink ("login") (modifier l'ID login en connexion)
...
//Fonctions pour confirmer/annuler la suppression d'une photo (transformer la const confirmeDelete en ConfirmDeleteelt)
